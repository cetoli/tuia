...
module ValidatesDateTime
  ...
  DEFAULT_TEMPORAL_VALIDATION_OPTIONS = {
    :before_message => "deve ser anterior a %s",
    :after_message  => "deve ser posterior a %s",
    :on => :save
  }.freeze
  ...
  module ClassMethods
    def validates_date(*args)
      options = temporal_validation_options({ :message => "é uma data inválida" }, args)
      attr_names = args
    ...
    def validates_time(*args)
      options = temporal_validation_options({ :message => "é uma hora inválida" }, args)
      attr_names = args
    ...
    def validates_date_time(*args)
      options = temporal_validation_options({ :message => "é uma data e hora inválida" }, args)
      attr_names = args
