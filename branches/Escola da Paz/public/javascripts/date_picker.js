...
function DatePickerEngine(form_el, link_el, display_as){
  ...
  /* Configuration options */
	...
	this.abbreviateMonthInLink = false;
	...
	this.days = 'Dom,Seg,Ter,Qua,Qui,Sex,Sab'.split(',');
	this.displayAs = display_as || function(date){
		...
		return(d_day + ' de ' + d_mon + ' de ' + d_year);
	};
  ...
  /* Method declarations */
	...
	this.writeCalendar = function(){
		...
		if(that.showCancelLink){
			o += '<tfoot><tr><td colspan="7"><div class="cancel_butt"><a href="javascript:void(0)" onclick="DatePicker.picker('+that.picker_id+').toggleDatePicker()">[x] cancel</a></div></td></tr></tfoot>';
		}
...
//		that.form_element.value = ansi_date;
		var ano = ansi_date.split('-')[0];
		var mes = ansi_date.split('-')[1];
		if (mes.length == 1) mes = '0' + mes;
		var dia = ansi_date.split('-')[2];
		if (dia.length == 1) dia = '0' + dia;
		that.form_element.value = ano + '-' + mes + '-' + dia;
